...
class Creature(object):
	...
	def __init__(self, name = " ", skills = [], elem = '<none>'):
		...
		self.equipTemp = {'weapon':'none','armor':'none',\
		'foot':'none'}
	...
	def __str__(self):
		return ("{0}-{1}: Level-{2}").format\
		(self.job.title(), self.name.title(),self.bstats[0])
	...
	def recover(self, s = 'hp', item = None):
	...
	def status(self):
			print('_'*30); print()
			info = ('{0} - {1}'.format(self.job.title(),\
			self.name.title()))
			sprint(info,'\t')
			print()
	...
	def agiToss(self,target,boolType='first'):
		...
		if boolType == 'first':
			...
			if faster == True:
				...
				x = (randint(1,100) % 5 == 0)
			lucky = 1 if (x == True or faster == True) else 0
			return lucky
